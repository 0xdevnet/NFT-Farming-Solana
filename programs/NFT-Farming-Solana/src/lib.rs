...
mod constants {
    pub const TOKEN_MINT_PUBKEY: &str = "D3dBYpFmEWJyDt4io19Kb3cNr6ShBCm6L4oFdZPBHYLb";
...
pub fn unDebitedPoints(
    ...
    return (current_timestamp as u128)
        .checked_sub(user_last_update_at as u128)
        .unwrap()
        .checked_mul(reward_per_token as u128)
        .unwrap()
...
pub mod nftfarming {
    ...
    pub fn stake(ctx: Context<Stake>, amount: u64) -> Result<()> {
        ...
        update_points_balance(pool, user_opt).unwrap();
    ...
    pub fn unstake(ctx: Context<Stake>, spt_amount: u128) -> Result<()> {
        ...
        let user_opt = Some(&mut ctx.accounts.user);
        update_points_balance(&mut ctx.accounts.pool, user_opt).unwrap();
    ...
    pub fn add_nft(ctx: Context<AddNFT>, price: u128) -> Result<()> {
        let pool = &mut ctx.accounts.pool;
    ...
    pub fn claim_nft(ctx: Context<ClaimNFT>, nft_id: u8) -> Result<()> {
        let total_staked = ctx.accounts.staking_vault.amount;
        ...
        println!(
            "redeemed? {}",
            ctx.accounts.pool.nfts[nft_id as usize].redeemed
        );
